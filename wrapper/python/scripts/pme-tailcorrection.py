...
"""
from Sire.Tools import PMEcutoff
...
parser.add_argument('-u', '--cutshort', nargs="?",
                    help="The cutoff distance used in the trajectory.")

parser.add_argument('-v', '--cutlong', nargs="?",
                    help="The long cutoff for reweighing.")

parser.add_argument('--use_solute_inter_nrg', action="store_true",
...
parser.add_argument('-k','--skip_frames',nargs="?",
                    help="List of intervals of frames to skip.")
...
if args.use_solute_inter_nrg:
    params["use solute intermolecular energy"]=True
#elif "use_solute_inter_nrg" in params:
#    use_solute_inter_nrg = params["use_solute_inter_nrg"]
#else:
#    use_solute_inter_nrg = False

if args.disable_crf:
    params["disable crf"]=True
...
if args.cutshort:
    params["short cutoff distance"] = float(args.cutshort) * angstrom

if args.cutlong:
    params["long cutoff distance"] = float(args.cutlong) * angstrom

if args.skip_frames:
    params["skip_frames"] = args.skip_frames
...
#Now lets run the RFcutoff correction code
PMEcutoff.runLambda(params)
